...
PlayerDamageTracker.prototype.addXPForUsers = function (redis, xp) {
    for (var i = 0; i < this.players_needing_xp.length; i++) {
        ...
        while (xpToLevel <= 0) {
            ...
            this.players_needing_xp[i].setMaxHP(this.players_needing_xp[i].getBaseMaxHP() + this.players_needing_xp[i].getHPUpdateForLevel());
            this.players_needing_xp[i].setHP(this.players_needing_xp[i].getBaseMaxHP());

            this.players_needing_xp[i].setStrength(this.players_needing_xp[i].getBaseStrength() + 1);
            if (this.players_needing_xp[i].getLevel() % 2 == 0) {
                this.players_needing_xp[i].setDexterity(this.players_needing_xp[i].getBaseDexterity() + 1);
            }
